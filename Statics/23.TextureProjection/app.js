...
var TextureCam = mat4.create();
var TextureProj = mat4.create();


var PhongRoutineIdx, DiffuseRoutineIdx;


var MatCameraAttr, LightAttrs, MaterialAttr, EyeAttr, NofLightAttr,TextureProjAtt;
var Cam;
var VBO;
...
function Init(Signal) {
    ...
    mat4.perspective(TextureProj, glMatrix.toRadian(30), 1, 0.1, 1000)
    mat4.lookAt(TextureCam,[30,30,30],[0,0,0],[0,0,-1])

    mat4.mul(TextureProj,TextureProj,TextureCam);    
    var m1 = mat4.create();
    mat4.fromScaling(m1,[.5,.5,.5]);
    mat4.mul(TextureProj,m1,TextureProj);
    mat4.fromTranslation(m1,[.5,.5,.5]);
    mat4.mul(TextureProj,m1,TextureProj);
    ...
    LightInfo = [{
            "Pos": new Float32Array([25, 25, 25, 1]),
    ...
    MaterialInfo = {
        "Ka": new Float32Array([0.2, 0.2, 0.2]), // Ambient
        "Kd": new Float32Array([0.8, 0.8, 0.8]), //Diffuse
        "Ks": new Float32Array([0.4, 0.4, 0.4]), //Specular
        "Shininess": 100
    }
...
function LoadData(Signal) {
    ...
    var P3 = new CPack({
        ...
        "FX": function () {
            ...
            TextureProjAtt = gl.getUniformLocation(GPUProgram, 'TextureProj');
            ...
            gl.uniformMatrix4fv(TextureProjAtt,gl.FALSE,TextureProj)
